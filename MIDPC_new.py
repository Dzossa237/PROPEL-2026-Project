...
if __name__=='__main__':
        ...
        net_evap = utils_new.customMPL(
                                insize=1*init.M+1+1*(nsteps)+1, outsize=init.M, hsizes=[200, 200, 200], 
                                nonlin=torch.nn.ReLU(), layer_norm=layer_norm, affine=affine_norm, dropout_prob=0.0,
                                mins=mins, maxs=maxs, u_min=init.T_evap_min, u_max=init.T_evap_max, 
                                clipping=False, spectral_norm=spectral_norm)

        net_integer = utils_new.customMPL(insize=1*init.M+1+1*(nsteps)+1+0, outsize=init.M-1, hsizes=[200, 200, 200],
        ...
        _, realized_loads_t = utils_new.generate_realized_load(
                                                        sampling_time=Ts,
                                                        nsteps=nsteps,
        ...
        load_forecast_t = utils_new.generate_forecast(realized_loads_t, training=True)

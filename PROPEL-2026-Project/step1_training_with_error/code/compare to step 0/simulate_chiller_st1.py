...
from utils_st1 import generate_load_forecast, generate_realized_load, plot_chiller_data, summary_table
...
if __name__=='__main__':
    ...
    if args.policy == 'RBC':
        ...
        policy = MIDPC_policy(
            load_path=f'/Final_Results_st1/N_15_Ts_180_M_2_15perc_30k_3b.pt',
            nsteps=args.nsteps,
    ...
    for cv_value in list_cv:
      ### Create and reshape realized load
      for index, seed_i in enumerate(list_seed):
        ...
        torch.save(outputs, f'/Final_Plots_st1/data_N{args.nsteps}_Ts_{init.Ts}_M_{init.M}_15perc_on{cv_value}_{index}.pt')
        
        if args.plotting:
            plot_chiller_data(outputs, Ts=init.Ts, time_unit='h',save_path=f'/Final_Plots_st1/data_N{args.nsteps}_Ts_{init.Ts}_M_{init.M}_15perc_on{cv_value}_{index}.pdf')
    ...
    df.to_csv("/Final_Plots_st1/summary_table_step1.csv", index=False)

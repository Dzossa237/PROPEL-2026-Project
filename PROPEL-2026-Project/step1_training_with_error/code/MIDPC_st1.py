#%%
import utils_st1; import torch
from init_st1 import SystemParameters
...
class MIDPC_policy():
        def __init__(self, nsteps, load_path, measure_inference_time = False):
                ...
                self.realized_load_filter_node = self.cl_system.nodes[1]   ### Add realized filter node and redefine indices
        ...
        def __call__(self, T_supply=None, T_return=None, load_forecast=None, filtered_load_forecast=None, realized_load = None):
                input_dict = {
                        'T_supply_and_return': torch.cat((T_supply,T_return), dim=-1).reshape(1,-1),
                        'load_forecast': load_forecast.reshape(1,-1),
                        'filtered_load_forecast': filtered_load_forecast[:,[0],:].reshape(1,-1),
                                }
...
if __name__=='__main__':
        ...
        net_evap = utils_st1.customMPL(
                                insize=1*init.M+1+1*(nsteps)+1, outsize=init.M, hsizes=[200, 200, 200], 
                                nonlin=torch.nn.ReLU(), layer_norm=layer_norm, affine=affine_norm, dropout_prob=0.0,
                                mins=mins, maxs=maxs, u_min=init.T_evap_min, u_max=init.T_evap_max, 
                                clipping=False, spectral_norm=spectral_norm)

        net_integer = utils_st1.customMPL(insize=1*init.M+1+1*(nsteps)+1+0, outsize=init.M-1, hsizes=[200, 200, 200],
                                        nonlin=torch.nn.ReLU(), layer_norm=layer_norm, affine=affine_norm, dropout_prob=0.,
                                        mins=mins, maxs=maxs, u_min=0., u_max=1., 
                                        clipping=False, spectral_norm=spectral_norm)

        # NEUROMANCER NODES
        load_forecast_filter_node = Node(load_filter, input_keys=['load_forecast'], output_keys=['filtered_load_forecast'])
        load_forecast_filter_node({'load_forecast': torch.zeros(1,1, device=device)})
       ...
       ### Create realized load filter node
        ...
        # NEUROMANCER SYSTEM
        cl_system = SystemPreview([load_forecast_filter_node, realized_load_filter_node, policy_integer_node, rounding_node, policy_evap_node, policy_flow_node , dynamics_node],
        ...
        load_forecast_variable = variable('load_forecast') # External
        filtered_load_forecast_variable = variable('filtered_load_forecast')
        ...
        #%% Dataloaders
        num_data = 70000; num_train_data = 60000; batch_size = 12000
        ...
        _, loads_t_1d = utils_st1.generate_load_forecast(
        ...
        print(loads_t.shape)
        realized_load = utils_st1.generate_realized_load(loads_t, nsteps, training=True)
        ...
        train_data = DictDataset({'T_supply_and_return':torch.cat((T_supply_t[:num_train_data].to(device),
                                ...
                                'load_forecast': loads_t[:num_train_data].to(device),
        ...
        torch.save(cl_system, f'/content/results/N_{nsteps}_Ts_{Ts}_M_{init.M}_60k_5_heteroskedastic_on_7.pt')
        torch.save(training_data, f'/content/results/training_data_N_{nsteps}_Ts_{Ts}_M_{init.M}_60k_5_heteroskedastic_on_7.pt')
# %%
...
class MIMPC_policy():
    ...
    # Define what parameters we need per call
    def __init__(self,
                 nsteps,
    ...
    def _build_model(self):
        ...
        # Define where the index for each variable starts
        ...
        # -----------------
        # Parameters that will be updated each call)
        # -----------------
        # Time-varying inputs
            ...
            for idx in v:
                if v[idx].value is None:
                    ...
                    if lb is not None and ub is not None:
                        v[idx].set_value(0.5 * (lb + ub), skip_validation=True)
                    else:
                        v[idx].set_value(0.0, skip_validation=True)
...
if __name__ == "__main__":
    ...
    load = torch.ones(1, nsteps, 1) * 400    # heat load tensor with value 400 at each time step n

    # create an instance of the MIMPC policy with different default parameters
    policy = MIMPC_policy(
        nsteps=nsteps,
    ...
    # Store the outputs of first call inside a dictionary 
    result = policy(
        T_supply=T_supply,
        T_return=T_return,
        load=load,
    ...
    flow_traj, integer_traj, T_evap_traj = policy.get_vals(policy.model, keys=('flow', 'integer', 'T_evap'))
    print(flow_traj)     # shape (N, M) — continuous mass flow per chiller per step
    print(integer_traj)  # shape (N, M) — binary on/off per chiller per step
    print(T_evap_traj)   # shape (N, M) — evaporator temperature per chiller per step

    # print("First call results:", result)
    # print("First call integer decisions:\n", result['integer'])
    # print("First call flow decisions:\n", result['flow'])
    # print("First call T_evap decisions:\n", result['T_evap'])
    # print("First call inference time (s):", result['inference_time'].item())
    # print("First call termination condition:", result['termination_condition'])
    # print("First call objective value:", policy.model.obj.expr())
    # print("First call chiller power:\n", policy.get_vals(policy.model, keys=('P_chiller',))[0])
    # print("First call pump power:\n", policy.get_vals(policy.model, keys=('P_pump',))[0])
    # print("First call COP:\n", policy.get_vals(policy.model, keys=('COP',))[0])
    # print("First call Q_delivered:\n", policy.get_vals(policy.model, keys=('Q_delivered',))[0])
    # print("First call T_supply trajectory:\n", policy.get_vals(policy.model, keys=('T_supply',))[0])
    # print("First call T_return trajectory:\n", policy.get_vals(policy.model, keys=('T_return',))[0])

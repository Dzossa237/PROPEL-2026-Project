...
import torch; from init_st0 import SystemParameters
from chiller_system_st0 import ChillerSystem
from utils_st0 import generate_datacenter_load, generate_realized_load, plot_chiller_data, summary_table
from utils_st0 import customMPL;
import pandas as pd
import time
...
if __name__=='__main__':
    ...
    if args.policy == 'RBC':
        ...
        from MIDPC_st0 import MIDPC_policy, round_fn, load_filter
        policy = MIDPC_policy(
            load_path=f'/content/results_step0/N_15_Ts_180_M_2.pt',
            nsteps=args.nsteps,
    ...
    # # # Load test
    seed = init.seed
    load_time, load_test, _ = generate_datacenter_load(number_of_days=args.n_days+1,
    ...
    ### 
    rows = []
    list_cv = [0.01, 0.03, 0.05, 0.07, 0.09, 0.11, 0.13, 0.15]
    list_seed = [303, 297, 373, 1193, 1787]
    for cv_value in list_cv:
      ## Reshape realized load
      for index, seed_i in enumerate(list_seed):
        realized_load = generate_realized_load(load_test, args.nsteps, seed_i, cv_value, training=False)
        ...
        torch.save(outputs, f'/Final_Plots_st0/data_N{args.nsteps}_Ts_{init.Ts}_M_{init.M}_on{cv_value}_{index}.pt')
        
        if args.plotting:
            plot_chiller_data(outputs, Ts=init.Ts, time_unit='h',save_path=f'/Final_Plots_st0/data_N{args.nsteps}_Ts_{init.Ts}_M_{init.M}_on{cv_value}_{index}.pdf')
        print("about to summarize, len(rows) =", len(rows))
        summary_table(outputs, cv_value, seed_i, rows, Ts=init.Ts)
        print("after summarize, len(rows) =", len(rows))

    df = pd.DataFrame(rows)
    df.to_csv("/Final_Plots_st0/summary_table_original_deployed_under_error3.csv", index=False)

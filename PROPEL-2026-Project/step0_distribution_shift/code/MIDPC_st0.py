...
if __name__=='__main__':
        ...
        net_evap = utils_st0.customMPL(
                                insize=1*init.M+1+1*(nsteps)+1, outsize=init.M, hsizes=[200, 200, 200], 
                                nonlin=torch.nn.ReLU(), layer_norm=layer_norm, affine=affine_norm, dropout_prob=0.0,
                                mins=mins, maxs=maxs, u_min=init.T_evap_min, u_max=init.T_evap_max, 
                                clipping=False, spectral_norm=spectral_norm)

        net_integer = utils_st0.customMPL(insize=1*init.M+1+1*(nsteps)+1+0, outsize=init.M-1, hsizes=[200, 200, 200],
        ...
        _, loads_t_1d,_ = utils_st0.generate_datacenter_load(
        ...
        torch.save(cl_system, f'/content/results_step0/N_{nsteps}_Ts_{Ts}_M_{init.M}.pt')
        torch.save(training_data, f'/content/results_step0/training_data_N_{nsteps}_Ts_{Ts}_M_{init.M}.pt')
# %%